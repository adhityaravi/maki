...
Replaces the pre-#102/#167 pattern of ``init()`` + module-level sentinels with
:class:`ImmuneClaudeReasoner` — dependencies land through ``__init__`` and
callers see a typed instance surface rather than ``Any``-typed globals.
"""
...
def _parse_pattern_classification(response: str) -> dict | None:
    ...
    or None if the block is missing or malformed. Pure function — kept at module
    scope so it stays trivially testable.
    """
    match = re.search(
...
def _build_escalation_summary(candidate: dict, classification: dict | None) -> str:
    ...
    Pure function — kept at module scope for the same reason as the parser above.
    """
...
class ImmuneClaudeReasoner:
    """The reasoning + escalation surface for maki-immune.

    Owns the semaphore, MCP server, KV handles, prompt template and every
    other collaborator the module used to reach through ``_nc`` / ``_config_kv``
    / ``_semaphore`` sentinels. ``main.py`` constructs one instance and wires
    its bound methods (``escalate_to_claude``, ``handle_immune_command``,
    ``cortex_stuck_handler``, ``immune_heartbeat_loop``, etc.) into NATS
    subscriptions and background loops.
    """
    ...
    def __init__(
        ...
        *,
        ...
        config_validators: dict[str, list],
        mcp_server: Any,
        semaphore: Any,
        system_prompt: str,
        publish_alert: Any,
        publish_vitals: Any,
        publish_immune_response: Any,
        recent_actions_max: int = 100,
        schedule_persist_recent_actions: Any = None,
        k8s_v1: Any = None,
        lock_kv: Any = None,
    ) -> None:
        self._nc = nc
        ...
        self._config_validators = config_validators
        self._mcp_server = mcp_server
        self._semaphore = semaphore
        self._system_prompt = system_prompt
        self._publish_alert = publish_alert
        self._publish_vitals = publish_vitals
        self._publish_immune_response = publish_immune_response
        self._schedule_persist = schedule_persist_recent_actions
        self._k8s_v1 = k8s_v1
        self._lock_kv = lock_kv
        # Per-instance loop-heartbeat cache — populated by
        # ``loop_heartbeat_watcher`` and read by ``_build_system_state``.
    ...
    def _build_system_state(self) -> str:
        ...
        for component, state in sorted(self._component_health.items()):
            ...
            if pod_name and pod_name in self._pod_metrics:
                m = self._pod_metrics[pod_name]
        ...
        if self._running_images:
            lines.append(f"\n## Local Images ({self._site_name})")
            for dep, tag in sorted(self._running_images.items()):
                lines.append(f"- {dep}: {tag}")

        if self._hive_state:
            lines.append(f"\n## Hive State ({len(self._hive_state)} peer(s) connected, local site: {self._site_name})")
            for site, state in sorted(self._hive_state.items()):
                ...
                if peer_images:
                    for dep, tag in sorted(peer_images.items()):
                        local_tag = self._running_images.get(dep)
                        drift = f" ⚠ DRIFT (local={local_tag})" if local_tag and local_tag != tag else ""
                        lines.append(f"  - {dep}: {tag}{drift}")
        else:
            lines.append(f"\n## Hive State (no peers connected, local site: {self._site_name})")

        if self._loop_heartbeats:
            lines.append("\n## Loop Heartbeats")
            now = time.time()
            for name, ts in sorted(self._loop_heartbeats.items()):
        ...
        escalation_block = self._render_recent_escalations()
        if escalation_block:
            lines.append("\n" + escalation_block)

        return "\n".join(lines)

    def _render_recent_escalations(self) -> str:
        """Render the recent Claude-escalation trail for the system-state prompt (#299).

        The full event stream lives in ``self._recent_actions`` (persisted to
        KV + gossiped via hive_state). Here we just filter for the escalation
        entries and pair started/finished rows so Claude can see, at a glance,
        whether the last N escalations actually completed or vanished into
        ``log.exception``. A run of "started" rows with no matching
        "complete"/"failed" is the exact smoking gun the #299 incident exposed.
        """
        ...
        escalations = [a for a in self._recent_actions if a.get("type") in escalation_types]
        if not escalations:
            return ""
    ...
    def _record_action(self, action: dict) -> None:
        """Append a structured action to recent_actions and persist (#299).

        Mirrors the pattern in health.py: append, trim, schedule persist, publish
        to ``IMMUNE_ACTION`` so hive peers see it too. Wrapped here so the
        reasoner doesn't have to know about the persistence wiring at every
        call site.
        """
        ...
        if self._schedule_persist is not None:
            try:
        ...
        if self._nc is not None:
            try:
                spawn_background(
                    self._nc.publish(IMMUNE_ACTION, json.dumps(action, default=str).encode()),
    ...
    async def _invoke_immune_claude(self, prompt_suffix: str = "") -> str:
        ...
        recent_actions_str = (
            json.dumps(self._recent_actions[-10:], indent=2, default=str) if self._recent_actions else "None"
        )
        ...
        response, _usage = await invoke_claude(
            ...
            mcp_servers={"maki-immune": self._mcp_server},
        )
        ...
        await apply_config_updates(
            self._config_kv,
            config_updates,
            allowed_keys=set(self._default_config.keys()),
    ...
    async def escalate_to_claude(self, component: str, state: dict, reason: str) -> None:
        ...
        entries in ``recent_actions`` — a ``claude_escalation_started`` row
...
Always report what you found and what you did via [DIGEST:...] and/or [ALERT:...]."""
            ...
            response = await self._invoke_immune_claude(suffix)

            digests = parse_tagged(response, "DIGEST")
            for digest in digests:
                await self._publish_vitals(digest)
            ...
            try:
                if self._publish_alert is not None:
            ...
            self._record_action(outcome)

    # --- Immune Command Handler ---

    async def handle_immune_command(self, msg) -> None:
        ...
        try:
            ...
            if cmd_lower in ("clear-blacklist", "clear blacklist"):
                cleared = list(self._failed_image_blacklist)
                self._failed_image_blacklist.clear()
                ...
                await self._publish_immune_response(message_id, reply)
                return
            if cmd_lower in ("show-blacklist", "show blacklist", "list-blacklist", "list blacklist"):
                if self._failed_image_blacklist:
                    reply = f"Blacklisted image tags ({len(self._failed_image_blacklist)}): " + ", ".join(
                        sorted(self._failed_image_blacklist)
                    )
                else:
...
Also use [DIGEST:...] for anything that should go to #maki-vitals."""
            ...
            try:
                ...
                await self._publish_immune_response(
    ...
    async def loop_heartbeat_watcher(self) -> None:
        ...
        while True:
            ...
            if self._lock_kv is None:
                continue
            now = time.time()
            new_heartbeats: dict[str, float] = {}
            try:
                keys = await self._lock_kv.keys()
            except Exception:
            ...
            for key in keys or []:
                ...
                try:
                    entry = await self._lock_kv.get(key)
                ...
                if now - ts > _LOOP_HEARTBEAT_TTL_S:
                    ...
                    try:
                        await self._lock_kv.delete(key)
                        log.info(
            ...
            self._loop_heartbeats = new_heartbeats

    # --- Immune Heartbeat Loop ---

    async def immune_heartbeat_loop(self) -> None:
        """Periodic holistic patrol with Claude reasoning."""
        log.info("Immune heartbeat loop started", extra={"instance_id": self._instance_id})
        ...
        while True:
            ...
            try:
                ...
                response = await self._invoke_immune_claude()
    ...
    async def escalate_pattern_to_claude(self, candidate: dict) -> None:
        ...
        known_patterns = await query_patterns(self._nc, component)
        if known_patterns:
            known_context = "\n".join(
...
- Be conservative: when in doubt, classify as escalate rather than no_op."""
        ...
        try:
            ...
            if classification:
                await write_pattern(self._nc, classification)
                log.info(
            ...
            await self._publish_alert(summary)
    ...
    async def passive_log_monitor_loop(self) -> None:
        ...
        """
        log.info("Passive log monitor loop started", extra={"instance_id": self._instance_id})
        ...
        while True:
            ...
            try:
                ...
                lock_key = f"loop.immune.passive_patrol.{self._site_name}"
                if not await try_claim_loop(self._lock_kv, lock_key, lock_ttl, self._instance_id):
                    continue
                ...
                if candidates:
                    ...
                    suppressed, to_escalate = await check_candidates(self._nc, candidates)
                    ...
                    for c in to_escalate:
                        ...
                        spawn_background(self.escalate_pattern_to_claude(c), name="immune.pattern_escalation")
    ...
    async def cortex_stuck_handler(self, msg) -> None:
        ...
        try:
            ...
            state = {
                ...
                "cortex_heartbeat_age_s": round(time.time() - self._cortex_state["last_heartbeat"], 1)
            ...
            spawn_background(
                self.escalate_to_claude("maki-cortex", state, reason),
